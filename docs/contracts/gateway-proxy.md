...
구현: [`SecurityConfiguration.java`](../../services/api-gateway-service/src/main/java/com/eevee/apigateway/config/SecurityConfiguration.java), [`ProxyTrustHeadersGatewayFilter.java`](../../services/api-gateway-service/src/main/java/com/eevee/apigateway/filter/ProxyTrustHeadersGatewayFilter.java), 설정 [`application.yml`](../../services/api-gateway-service/src/main/resources/application.yml)의 `gateway.dev-mode`·`gateway.jwt.secret`.
...
| 헤더 | 필수 | 설명 |
|------|------|------|
| `X-User-Id` | 예 | 플랫폼 사용자 ID (JWT `sub` 등) |
...
### 4.2 Usage 원장 `user_id`와 `X-User-Id`·JWT `sub` 정합 (팀 B·C·게이트웨이)
...
- **운영(`gateway.dev-mode=false`)** 게이트웨이는 플랫폼 JWT 검증 후 **`X-User-Id` = JWT `sub`** 를 설정한다(구현: `ProxyTrustHeadersGatewayFilter.forwardWithJwt`).
...
- **로컬 `GATEWAY_DEV_MODE=true` + Web BFF** 는 Identity `GET /api/auth/session`의 **`email`** 로 `X-User-Id`를 보강한다(`apps/web` Usage BFF). 따라서 **개발 모드에서 붙는 `X-User-Id`와 운영 JWT `sub`는 동일 규칙(이메일)** 으로 맞춰져, Proxy→Usage 원장과 대시보드 조회 키가 어긋나지 않는다.
- **회귀 테스트:** `ProxyTrustHeadersGatewayFilterTest`(api-gateway-service) — JWT `sub` → `X-User-Id`, 개발 모드·익명 컨텍스트 시 인바운드 `X-User-Id` 유지.
...
## 9. 운영·Web BFF 베이스 URL (팀 A·C 합의)

- **`API_GATEWAY_URL`** (`apps/web` 환경 변수): 브라우저·Next.js BFF가 **호출하는 API Gateway의 공개 베이스 URL**이다(스킴+호스트+포트, trailing path 없음). 예: 로컬 `http://localhost:8080`, 운영 `https://<배포된 게이트웨이 호스트>`. Usage·AI 요청은 이 베이스에 `/api/v1/usage/...`, `/api/v1/ai/...` 를 붙인다.
...
- 로컬에서 Identity와 게이트웨이 **포트가 겹치면** `IDENTITY_SERVICE_URL`과 `API_GATEWAY_URL`을 팀이 분리해 맞춘다([`apps/web/.env.example`](../../apps/web/.env.example) 주석).

운영 배포 시 **공개 DNS·TLS 종단점**이 가리키는 URL을 팀 A(게이트웨이)·팀 C(웹/BFF)가 동일 값으로 두는 것을 합의 기준으로 한다.
