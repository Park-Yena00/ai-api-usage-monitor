...
- 본 저장소의 웹 UI·BFF는 **`apps/web/`** 에 둔다 (App Router, Route Handler로 Identity 프록시 등).
- **인증 쿠키·BFF 엔드포인트·401 처리** 계약: [`docs/contracts/web-identity-bff.md`](contracts/web-identity-bff.md).
- **게이트웨이 경유 Usage·AI:** Bearer 검증·`RemoveRequestHeader=Authorization`·운영 `API_GATEWAY_URL` 합의는 [`docs/contracts/gateway-proxy.md`](contracts/gateway-proxy.md) §1.1·§3·§9(팀 A·C).
- **어떤 URL이 로그인 없이 막히는지**(미들웨어 matcher, 대응 `[[...path]]` 페이지, JWT 미검증 주의): 동 문서 **§6.2**.
- **공개 랜딩·진입점**: 루트 페이지 `src/app/page.tsx`에서 `/login`·`/signup` 링크를 둔다(상세: 동 계약 **§10**). **회귀 테스트**는 `src/app/api/auth/**/route.test.ts`, `middleware.test.ts` 등(Vitest, 동 문서 **§10.2**).
